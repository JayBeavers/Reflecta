...
process.on('uncaughtException', function(err) { console.log('Exception: ' + err); });
...
var readUnescaped = function() {
    ...
    if (escaped) {
      switch (b)
      {
        ...
        default:
          // Todo, emit a frame corrupt error
    ...
    {    
      ...
      if (b == EscapeCharacters.END) { // end of frame detected, process the data accumulated in the frameBuffer
        ...
        if (readChecksum != 0) {
          ...
          console.log(new Buffer(frameBuffer));
...
serialPort.on('close', function() { console.log('close'); });
serialPort.on('end', function() { console.log('end'); });
serialPort.on('error', function(err) { console.log('error '  + err); });

// Simple test code to generate a signal and response
//var loop = function() {
//  sendFrame([1]); // Sends a call to QueryInterface when using BasicReflecta on the Arduino
